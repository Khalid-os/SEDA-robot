...
//AQUI LA CAGARON LOS PROFES Y DONDE PONE BOTON AUMENTAR ENREALIDAD ES EL - Y EL BOTON DISMINUIR ES EL +, NO HAGAS CASO AL COMENTARIO
...
int luz1,luz2;
float luz_diferencia,luz_media; //porcentaje de luz (luz1 y luz2)
int a=0;//contador para mostrar datos cada segundo(en TIM0)
int vi,vd,vi_aux,vd_aux;//velocidad derecha,velocidad izquierda
float v_1,v_2;//tensiones recogidas por el adc
/* Timer 0 interrumpe cada 1/Fs */
float umbral = 20;

void TIMER0_IRQHandler(void)
{	
	if((LPC_TIM0->IR & 0x2)==0x2){
		a++;
   LPC_ADC->ADCR|=(1<<16); // BURST=1 --> Cada 65TclkADC se toma una muestra de cada canal comenzando 
	...
	LPC_TIM0->IR|=(1<<1);   // Borrar flag interrupción	
		if(a==10){
			v_1=canal_2*3.3/4095; //mostramos la tension cada segundo
			v_2=canal_5*3.3/4095;
			a=0;
			luz1=v_1*1000/30;//sacamos el porcentaje de luz, vmax= 3V
			luz2=v_2*1000/30;
			luz_media=(luz1+luz2)/2;
			luz_diferencia=abs(luz1-luz2);
		}
		//si la luz supera un 20% del umbral se activan los motores y
		//si existe una direncia mayor al 5% se producen los giros
		if((luz1<20)&&(luz2<20)){ 
			vd=0;                  //no hay suficiente intensidad se para el robot                                          
			vi=0;
		}
		if((luz_diferencia>3)&&((luz1>20)||(luz2>20))){
				vd=luz1;
			  vi=luz2;
		}
		if((luz_diferencia<3)&&((luz1>20)||(luz2>20))){
