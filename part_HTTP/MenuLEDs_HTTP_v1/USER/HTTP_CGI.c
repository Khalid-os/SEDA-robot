...
extern int velDerecha;
extern int velIzquierda;
...
void cgi_process_var (U8 *qs) {
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (str_scomp (var, "velderecha=") == __TRUE) {
         sscanf ((const char *)&var[12], "%d%d",&velDerecha);
      }
      else if (str_scomp (var, "velizq=") == __TRUE) {
         sscanf ((const char *)&var[8], "%d%d",&velIzquierda);
      }
      else if (str_scomp (var, "umbral=") == __TRUE) {
         sscanf ((const char *)&var[8], "%d%d",&distancia);
...
//          len = sprintf((char *)buf,(const char *)&env[4],led_1 ? "checked" : "");
//          break;
//        case '2':
//          /* Write local Net mask. */
//          len = sprintf((char *)buf,(const char *)&env[4],led_2 ? "checked" : "");
//          break;
//        case '3':
//          /* Write the local IP address. The format string is included */
//          /* in environment string of the script line.                 */
//          len = sprintf((char *)buf,(const char *)&env[4],led_1 ? "red" : "blue");
//          break;
//        case '4':
//          /* Write local Net mask. */
//          len = sprintf((char *)buf,(const char *)&env[4],led_2 ? "red" : "blue");
//          break;
//        default:
//           break;
//      }
